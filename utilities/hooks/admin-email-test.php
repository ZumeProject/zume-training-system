...
class Zume_Email_Test_Admin {
    ...
    /**
     * @var Zume_Admin_Cron The admin cron instance for testing
     */
    private $admin_cron_instance;
    ...
    public function __construct() {
        ...
        if ( ! class_exists( 'Zume_Admin_Cron' ) ) {
            add_action( 'admin_notices', function () {
                echo '<div class="notice notice-error"><p>Zume_Admin_Cron class not found. Please ensure the zume-admin-cron.php file is loaded.</p></div>';
            });
            return;
        }
        ...
        $this->admin_cron_instance = new Zume_Admin_Cron();
    ...
    public function handle_form_submission() {
        if ( ! isset( $_POST['zume_email_test_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['zume_email_test_nonce'] ) ), 'zume_email_test_action' ) ) {
            return;
        }

        // Handle cron test submission
        if ( isset( $_POST['run_cron_test'] ) && isset( $_POST['cron_test_date'] ) ) {
            $this->run_cron_test( sanitize_text_field( wp_unslash( $_POST['cron_test_date'] ) ) );
        }
        ...
        // Handle admin cron test
        if ( isset( $_POST['run_admin_cron_test'] ) ) {
            $this->run_admin_cron_test();
        }
    }
    ...
    private function display_test_results( $results, $test_type ) {
        add_action( 'admin_notices', function () use ( $results, $test_type ) {
            ...
            if ( $results['processed'] > 0 ) {
                echo '<div class="notice notice-success"><p>Successfully processed ' . esc_html( $results['processed'] ) . ' messages.</p></div>';
            } else {
            ...
            // Summary
            echo '<div class="notice notice-info">';
            echo '<p><strong>Summary:</strong> Found ' . esc_html( $results['messages_found'] ) . ' eligible messages, processed ' . esc_html( $results['processed'] ) . ' successfully.</p>';
            echo '</div>';
        });
    }

    /**
     * Display admin cron test results in admin notices
     *
     * @param array $results The results from the admin cron test
     */
    private function display_admin_cron_results( $results ) {
        add_action( 'admin_notices', function () use ( $results ) {
            echo '<h3>' . esc_html( 'Zume Admin Cron Test Results' ) . '</h3>';
            if ( is_array( $results ) && count( $results ) > 0 ) {
                ...
                if ( $success_count > 0 ) {
                    echo '<div class="notice notice-success"><p>Successfully sent ' . esc_html( $success_count ) . ' admin messages.</p></div>';
                }
                if ( !empty( $error_msgs ) ) {
                    echo '<div class="notice notice-error">';
                    echo '<p><strong>Errors encountered:</strong></p>';
                ...
                if ( $success_count === 0 && empty( $error_msgs ) ) {
                    echo '<div class="notice notice-warning"><p>No admin messages were sent.</p></div>';
                }
            } else {
                echo '<div class="notice notice-warning"><p>No admin messages were sent.</p></div>';
            }
            ...
            <div class="metabox-holder">
                <div class="postbox-container" style="width: 100%;">
                    <div class="meta-box-sortables">
                        ...
                        <!-- Admin Cron Test -->
                        <div class="postbox">
                            <h2 class="hndle"><span>Admin Cron Job Test</span></h2>
                            <div class="inside">
                                <form method="post" action="">
                                    <?php wp_nonce_field( 'zume_email_test_action', 'zume_email_test_nonce' ); ?>
                                    <p class="description">
                                        This will run the Zume Admin Cron job, sending admin messages to all users with the 'zume_admin' role.<br>
                                        <strong>Note:</strong> This actually sends emails!
                                    </p>
                                    <p class="submit">
                                        <input type="submit"
                                               name="run_admin_cron_test"
                                               id="run_admin_cron_test"
                                               class="button button-secondary"
                                               value="Run Admin Cron Test">
                                    </p>
                                </form>
                            </div>
                        </div>
