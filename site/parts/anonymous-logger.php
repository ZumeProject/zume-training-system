...
function zume_content_logger( $type, $subtype, $language_code ) {
    $log_endpoint = ( is_user_logged_in() ) ? 'log' : 'log_anonymous';
    ?>
    <script>
        ...
        jQuery(document).ready(function(){
            let log_endpoint = '<?php echo esc_attr( $log_endpoint ) ?>'
            ...
            jQuery(document).scroll(function() {
                if (jQuery(document).scrollTop() >= 200 && has_scrolled === false ) {
                    zumeRequest.post( log_endpoint, { type: type, subtype: subtype, language_code: language_code } )
                        .then((log) => {
                            console.log(log)
                        })
                        .catch((error) => {
